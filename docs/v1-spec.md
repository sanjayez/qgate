...
## Text And HTML Report Contracts

### `test-plan.md`

Generated by `qgate plan`. It must include:

- inferred PR intent and confidence
- impacted surfaces
- non-happy-path obligations grouped by risk id
- required checks for each risk
- source paths for each risk
- security mappings when available

### `gate-report.md`

Generated by `qgate run` or `qgate report`. It must include:

- verdict, gate mode, run id, artifact directory, and generation time
- intent summary and confidence
- critical, warning, info, and blocker counts
- blocker ids and reasons
- impact summary
- top risks

### `gate-report.html`

Generated from `gate-report.md`. It must preserve the same report content in a
browser-readable HTML document. HTML rendering is presentation-only; the Markdown
and JSON artifacts remain the reviewable source of truth.

## Future Execution Artifact

### `execution-results.json`

Planned for the execution gate phase. It should normalize test runner, scanner,
browser, API, and contract-check output into structured results with source,
status, severity, command, duration, and evidence excerpts.
